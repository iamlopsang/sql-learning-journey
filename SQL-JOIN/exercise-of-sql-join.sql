...
-- total sales.
    ...
    -- a. Display the movie title, length in minutes, and international sales.

    SELECT m.title, m.length_minutes, b.international_sales
    FROM movies AS m
    INNER JOIN boxoffice AS b
    ON m.id = b.movie_id;

-- Explanation: Join both tables using the movie ID to display the movie title, length, and
-- international sales.
...
-- Explanation: Join both tables and display movies with international sales greater than 400 million.
    ...
    -- d. Find movies directed by Brad Bird and display their title and domestic sales.

    SELECT m.title, b.domestic_sales
    FROM movies AS m
    INNER JOIN boxoffice AS b
    ON m.id = b.movie_id
    WHERE m.director = 'Brad Bird';
...
-- Explanation: Join both tables, calculate total sales, and show movies whose total sales are 
